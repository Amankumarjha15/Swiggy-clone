...
function Search() {
  ...
  return (
    ...
    <input  onKeyDown={handleSearchData} type="text" placeholder='Search Your Favorite Food From Here' />
...
{
  DishesData&&
   DishesData.map(({card : {card : {info : {imageId = "" ,name:dish, price , isVeg = 0 } , restaurant : {info : {name:resname , id , avgRating , sla : {slaString}}}}}})=>(
      ...
      <div className='w-[90%] h-[250px] m-5 bg-white text-black rounded-3xl'>
          ...
          <div className="h-[70%] m-5 flex justify-between">
            ...
            <button  className="bg-white absolute bottom-[20px] left-1/2 -translate-x-1/2 text-lg font-bold rounded-2xl border px-5 py-1 drop-shadow text-green-700">ADD</button>
            </div>
          </div>
